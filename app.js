const ROSTER_KEY = "guardianTapRosterV1";
const RECORD_KEY = "guardianCheckins";

const state = { grade:null, className:null, initial:null, student:null, guardian:null };
...
function saveRoster(v){localStorage.setItem(ROSTER_KEY,JSON.stringify(v));}
function records(){ try{return JSON.parse(localStorage.getItem(RECORD_KEY)||"[]");}catch{return [];} }
...
function normalizeRoster(rows){
  const required=["grade","class","name","surnameKana"];
  if(!rows.length||required.some(k=>!(k in rows[0]))) throw new Error("CSVの列名が正しくありません。");
  const out=rows.map(r=>({grade:String(r.grade||"").replace(/年/g,"").trim(),className:String(r.class||"").replace(/組/g,"").trim(),number:String(r.number||"").trim(),surname:String(r.surname||"").trim(),surnameKana:toKatakana(String(r.surnameKana||"").trim()),name:String(r.name||"").trim(),kana:toKatakana(String(r.kana||"").trim())})).filter(r=>r.grade&&r.className&&r.name&&r.surnameKana);
  if(!out.length)throw new Error("読み込める生徒データがありません。");
  return out;
}
function kanaGroup(kana){
  const c=toKatakana(kana).replace(/[^ァ-ヶ]/g,"")[0]||"他";
  const groups=[["あ","アイウエオァィゥェォ"],["か","カキクケコガギグゲゴ"],["さ","サシスセソザジズゼゾ"],["た","タチツテトダヂヅデドッ"],["な","ナニヌネノ"],["は","ハヒフヘホバビブベボパピプペポ"],["ま","マミムメモ"],["や","ヤユヨャュョ"],["ら","ラリルレロ"],["わ","ワヲンヮ"]];
  return groups.find(([,chars])=>chars.includes(c))?.[0]||"他";
}
function resetFlow(){state.grade=null;state.className=null;state.initial=null;state.student=null;state.guardian=null;}

function render(){
  adminMode=false; const list=roster();
  if(!list.length)return renderSetup();
  if(!state.grade)return renderGrades(list);
  if(!state.className)return renderClasses(list);
  if(!state.initial)return renderInitials(list);
  if(!state.student)return renderStudents(list);
  if(state.guardian===null)return renderGuardians();
  return renderCompanions();
}
function renderSetup(message=""){
  resetFlow();
  app.innerHTML=`<section class="setup-card"><div class="step-kicker">はじめに1回だけ</div><h2 class="step-title">在校生名簿を<br>読み込んでください</h2><p class="step-sub">受付用CSVをこの端末で選びます。氏名はGitHubへ送信されず、この端末のブラウザ内だけに保存されます。</p>${message?`<div class="notice">${esc(message)}</div>`:""}<label class="file-btn" for="rosterFile">📄 名簿CSVを選ぶ</label><input id="rosterFile" class="visually-hidden" type="file" accept=".csv,text/csv"><div class="privacy-note">🔒 公開ページには実名簿を保存しません</div></section>`;
  document.getElementById("rosterFile").addEventListener("change",async e=>{const file=e.target.files?.[0];if(!file)return;try{const normalized=normalizeRoster(parseCSV(await file.text()));saveRoster(normalized);resetFlow();render();}catch(err){renderSetup(`読み込みに失敗しました：${err.message}`);}});
}
function renderGrades(list){
  const grades=[...new Set(list.map(r=>r.grade))].sort((a,b)=>Number(a)-Number(b));
  app.innerHTML=`<div class="step-kicker">STEP 1 / 6</div><h2 class="step-title">お子さまの学年を<br>押してください</h2><p class="step-sub">大きなボタンを1回押します。</p><div class="grid">${grades.map(g=>`<button class="big-btn primary" data-grade="${esc(g)}">${esc(g)}年</button>`).join("")}</div>`;
  app.querySelectorAll("[data-grade]").forEach(b=>b.onclick=()=>{state.grade=b.dataset.grade;render();});
}
function renderClasses(list){
  const classes=[...new Set(list.filter(r=>r.grade===state.grade).map(r=>r.className))].sort((a,b)=>Number(a)-Number(b));
  app.innerHTML=`<div class="step-kicker">STEP 2 / 6</div><h2 class="step-title">${esc(state.grade)}年の<br>クラスを押してください</h2><p class="step-sub">お子さまのクラスを選びます。</p><div class="grid four">${classes.map(c=>`<button class="big-btn secondary" data-class="${esc(c)}">${esc(c)}組</button>`).join("")}</div>`;
  app.querySelectorAll("[data-class]").forEach(b=>b.onclick=()=>{state.className=b.dataset.class;render();});
}
function renderInitials(list){
  const students=list.filter(r=>r.grade===state.grade&&r.className===state.className); const allGroups=["あ","か","さ","た","な","は","ま","や","ら","わ","他"]; const groups=allGroups.filter(g=>students.some(s=>kanaGroup(s.surnameKana)===g));
  app.innerHTML=`<div class="step-kicker">STEP 3 / 6</div><div class="choice-summary">${esc(state.grade)}年 ${esc(state.className)}組</div><h2 class="step-title">名字の最初の音を<br>押してください</h2><p class="step-sub">例：山田さん →「や」　鈴木さん →「さ」</p><div class="kana-grid">${groups.map(g=>`<button class="kana-btn" data-initial="${g}">${g}</button>`).join("")}</div>`;
  app.querySelectorAll("[data-initial]").forEach(b=>b.onclick=()=>{state.initial=b.dataset.initial;render();});
}
...
function complete(companion){
  const total=state.guardian+companion; const rec={id:crypto.randomUUID?crypto.randomUUID():String(Date.now())+Math.random(),date:new Date().toISOString(),grade:state.grade,className:state.className,student:state.student.name,number:state.student.number,guardians:state.guardian,companions:companion,total}; const all=records();all.push(rec);saveRecords(all);
  app.innerHTML=`<div class="success"><div class="check">✓</div><h2>受付完了</h2><div class="total">合計 ${total}名</div><p>保護者 ${state.guardian}名　＋　同行者 ${companion}名</p><p>そのままお進みください</p></div>`;
  setTimeout(()=>{resetFlow();render();},1500);
}
function downloadRecords(){
  const all=records(); const header=["日時","学年","組","生徒名","出席番号","保護者","同行者","合計"]; const lines=[header,...all.map(r=>[r.date,r.grade,r.className,r.student,r.number||"",r.guardians,r.companions,r.total])].map(row=>row.map(v=>`"${String(v??"").replace(/"/g,'""')}"`).join(",")); const blob=new Blob(["\uFEFF"+lines.join("\r\n")],{type:"text/csv;charset=utf-8"}); const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`guardian-checkin-${new Date().toISOString().slice(0,10)}.csv`;a.click();URL.revokeObjectURL(a.href);
}
